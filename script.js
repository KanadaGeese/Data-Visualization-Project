...
const TEAM_MAP = {
  ...
  // Load CSV data
  let data;
  d3.csv("nba_player_stats_cleaned.csv", d3.autoType).then(dataset => {
    data = dataset;
  ...
  function updateAll(p = null, s = null) {
    ...
    // Update player image and info fetching with wikipedia
    ...
    const wikiName = playerRow.Player.replace(" ", "_"); //JS01

    fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${wikiName}`) //JS02
  ...
  //In case the wikipedia page is unfoundable
  ...
  function drawMap(player) {
    ...
    // Loads the different part of the world & USA States
    Promise.all([
      d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson"),
      d3.json("https://cdn.jsdelivr.net/npm/us-atlas@3/states-10m.json")
    ]).then(([world, us]) => {
    // Converts to an SVG object
      svg.append("g")
        .selectAll("path")
    ...
    //Plot cities on the map based on the lattitude and longitude
  ...
  //Fetches similar Players and adds them to compare for graphs
  function drawComparison(playerRow) {
    ...
    // Finds the most similar players by points
...
fetch('nba_player_stats_cleaned.csv')
  .then(res => res.text())
  .then(data => {
...
// Draws the player radar in the solo section
